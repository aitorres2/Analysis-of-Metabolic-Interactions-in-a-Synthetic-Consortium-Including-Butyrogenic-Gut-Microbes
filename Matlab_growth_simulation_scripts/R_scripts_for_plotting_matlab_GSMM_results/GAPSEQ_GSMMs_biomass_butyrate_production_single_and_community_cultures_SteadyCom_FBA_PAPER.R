...
# Read the TSV file that contains the iris dataset
iris <- read_tsv("iris_dataset.tsv")


# procesando sampling
setwd("/home/alexis/Desktop/GITHUB_REspaldos_linx_2025/Objective_1_Metabolic_interactions_insilico/Simulation_of_growth/R_scripts_for_plotting_matlab_GSMM_results")

diseño_consorcios_data <- read_excel("design_consortium_modelseed_vFINAL.xlsx", sheet = 1, col_names = TRUE)
...
symbiosum_subset <- diseño_consorcios_df %>% 
  filter(Consortium %in% grep("WAL", diseño_consorcios_df$Consortium, value = TRUE)) 
...
#----------------------------------------------------------------------------------------------------------------------------------
mono_subset$Butyrate <- as.numeric(mono_subset$"Butyrate cpd00211")
innocuum_subset$Butyrate <- as.numeric(innocuum_subset$"Butyrate cpd00211")
symbiosum_subset$Butyrate <- as.numeric(symbiosum_subset$"Butyrate cpd00211")
saccharolyticum_subset$Butyrate <- as.numeric(saccharolyticum_subset$"Butyrate cpd00211")
...
# Save figure
ggsave("final_plot_gapseq_2_PAPER.pdf", plot = final_plot, width = 20, height = 12, units = "in", dpi = 320, bg = "white")
ggsave("final_plot_gapseq_2_PAPER.png", plot = final_plot, width = 20, height = 12, units = "in", dpi = 320, bg = "white")
